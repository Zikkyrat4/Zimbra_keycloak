...
- ✅ Sign Assertions = ON
- ✅ Force POST Binding = ON
- ✅ Тестовый пользователь создан (test@mail.prokopenko.com)
- ✅ SAML расширение samlextn.jar установлено в Zimbra
- ✅ /opt/zimbra/conf/saml/saml-config.properties создан
- ✅ Сертификат Keycloak добавлен в домен Zimbra
- ✅ zimbraWebClientLoginURL установлен на SAML endpoint
- ✅ Zimbra перезагружена (zmmailboxdctl restart)
- ✅ SAML вход работает в браузере
...
| **Zimbra Web UI** | https://mail.prokopenko.com |
...
| **SAML Logout** | https://mail.prokopenko.com/service/extension/samlslo |
...
1. **Создать других пользователей в Keycloak** и проверить вход
2. **Настроить Single Logout (SLO)** - выход из Zimbra разлогиняет из Keycloak
3. **Включить проверку подписей** для продакшена (изменить saml_skip_signature_validation на false)
4. **Интегрировать LDAP** в Keycloak для синхронизации пользователей
5. **Настроить автоматическое создание пользователей** при первом входе
...
1. **Логи Zimbra:** `/opt/zimbra/log/zmmailboxd.out`
2. **Логи Keycloak:** Консоль Keycloak или логи контейнера
3. **SAML метаданные:** http://192.168.1.105:8080/realms/zimbra/protocol/saml/descriptor
4. **README расширения:** `/opt/zimbra/extensions-network-extra/saml/README.txt`
5. **Официальная документация:** https://wiki.zimbra.com/
